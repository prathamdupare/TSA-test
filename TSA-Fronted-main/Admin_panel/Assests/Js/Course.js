...
async function fetchBlogs() {
  ...
  try {
    ...
    blogsArr = blogs.courses;
    renderBlogs(blogs.courses);
  } catch (error) {
    ...
    <div class="bg-white border border-gray-200 max-h-[365px]  rounded-lg shadow hover:shadow-lg">
      ...
      <div class="p-5 ">
        ...
        <div class=" text-gray-500 flex items-center justify-between">
          ...
          <p class-"text-right"><span class="font-bold text-gray-500 mr-1">Price: </span> ${blog?.price || "Not specified"}</p>
...
async function deleteSession(Session) {
  try {
    ...
    const response = await fetch(`${apiUrl}/api/course/${Session.id}`, {
      method: 'DELETE',
      headers: {
