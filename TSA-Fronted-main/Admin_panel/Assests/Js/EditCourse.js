...
let sessionId, sessionName,  sessionPrice, sessionSelfHelpCredit, sessionSessions, sessionDescription;
...
document.getElementById('editSessionForm').addEventListener('submit', async (event) => {
  ...
  const currentSessionDescription = quill.root.innerHTML;
  ...
  if (confirm('Are you sure you want to update this Course?')) {
    try {
      const response = await fetch(`${apiUrl}/api/course/${sessionId}`, {
        method: 'PUT',
